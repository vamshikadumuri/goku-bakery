...
# For HF datasets (-d): full dataset repo download into /datasets/<name> in a single image.
...
print_usage() {
  ...
  echo "  -d              Treat -m as a HF dataset repo (baked into /datasets/)"
  echo "  -h              Help"
}
...
hf_download_repo() {
  ...
  REPO="$REPO" LOCAL_DIR="$LOCAL_DIR" IGNORE="$IGNORE" REPO_TYPE="$REPO_TYPE" \
  $VENV_PYTHON - <<'PYEOF'
import os, sys
...
print(f'  Downloading {repo_type} repo {repo} -> {local_dir}' + (f'  (ignoring {ignore})' if ignore else ''))
snapshot_download(repo, repo_type=repo_type, local_dir=local_dir, ignore_patterns=ignore)
...
verify_image_contents() {
  local IMAGE_TAG=$1 SHARD_DIR=$2 ROOT=${3:-models}
  echo "   🔍 Verifying image contents..."
  EXPECTED=$(cd "$SHARD_DIR" && find "$ROOT/" -type f | sort)
  VERIFY_OUTPUT=$(docker run --rm "$IMAGE_TAG" sh -c "find /$ROOT/ -type f | sort; echo '===SEP==='; du -sh /$ROOT/" 2>/dev/null)
  ...
  echo "   ✅ Verified: $(echo "$ACTUAL" | wc -l) files, ${IMAGE_SIZE} in /${ROOT}/"
  return 0
}
...
build_and_push_shard() {
  local SHARD_NUM=$1 TAG=$2 SHARD_DIR=$3 ROOT=${4:-models}
  echo "   🔨 Building ${IMAGE_NAME}:${TAG}..."
  if ! docker build -t "${IMAGE_NAME}:${TAG}" "$SHARD_DIR"; then
    echo "   ❌ Build FAILED"; return 1
  fi
  if ! verify_image_contents "${IMAGE_NAME}:${TAG}" "$SHARD_DIR" "$ROOT"; then
...
# ── Dataset path: full dataset repo → single image under /datasets/<name> ─────
if [[ "$REPO_TYPE" == "dataset" ]]; then
  TAG="${TAG_PREFIX}"
  SHARD_DIR="$WORK_DIR/shard1"
  SHARD_DATA_DIR="$SHARD_DIR/datasets/$MODEL_NAME"
  mkdir -p "$SHARD_DIR/models" "$SHARD_DATA_DIR"
  echo "$DOCKERFILE" > "$SHARD_DIR/Dockerfile"

  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "📦 ${IMAGE_NAME}:${TAG} (dataset — full repo)"
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  if ! hf_download_repo "$MODEL_REPO" "$SHARD_DATA_DIR" "$EXCLUDE_PATTERNS"; then
    echo "❌ Download failed"; perm_rm "$WORK_DIR"; exit 1
  fi
  rm -rf "$SHARD_DATA_DIR/.huggingface" "$SHARD_DATA_DIR/.cache"

  if ! build_and_push_shard "1" "$TAG" "$SHARD_DIR" "datasets"; then
