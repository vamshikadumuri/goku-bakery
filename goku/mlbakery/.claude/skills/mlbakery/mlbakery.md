...
- **Dataset** (`-d`): always a single image, tag `family-datasetname`, e.g. `JailbreakBench/JBB-Behaviors` → `JailbreakBench-JBB-Behaviors`
...
| `-d` | Treat `-m` as a HF **dataset** repo | off (model) |
| `-h` | Show help | — |
...
```bash
bash scripts/bake.sh -d -m JailbreakBench/JBB-Behaviors
```

With `-d`, the whole dataset repo is downloaded (`repo_type="dataset"`) into
